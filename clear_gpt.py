...
def process_cookie_file(cookie_file_path: Path):
    ...
    try:
        ...
        # 3. Locate and click 'Delete all chats' button
        print("[STEP] Locating 'Delete all' chats button...", flush=True)
        delete_all_btn = page.get_by_role('button', name='Delete all Delete all chats')
        
        # Element visible hone tak ka thoda explicit wait (max 10s) taaki crash na ho
        delete_all_btn.wait_for(state="visible", timeout=10000)
        print("[STEP] Clicking 'Delete all' button...", flush=True)
        delete_all_btn.click()

        # Thoda sa pause modal animation load hone ke liye
        time.sleep(2)

        # 4. Locate and click Confirmation button (Using fallback conditions)
...
def run():
    ...
    # Har ek encrypted file ko loop me ek ek karke chala rhe hain
    for index, cookie_file in enumerate(encrypted_files, start=1):
        ...
        # Optional: Agli cookie file uthane se pehle 5 second ka break (taaki session clean up ho sake)
