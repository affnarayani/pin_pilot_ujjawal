...
# Camoufox से Sync Api को इम्पोर्ट कर रहे हैं
...
# =========================
def run():
    print("[START] Script started with Camoufox Engine", flush=True)
    ...
    try:
        with Camoufox(
            ...
            # Trigger action if found (FORCED CLICK & FILL)
            if textbox.count() > 0:
                textbox.first.click(force=True)
                print("[OK] Textbox located and clicked successfully (Forced).", flush=True)
            else:
            ...
            # ========================================================
            # SEND BUTTON LOCATORS WITH STRATEGIC FORCE CLICK
            # ========================================================
            ...
            # ========================================================
            # STABLE Live Stream JSON Parsing with Multiple Fallbacks (CRITICAL FIX)
            # ========================================================
            print("[STEP] Waiting for generated JSON code block to complete writing (15s checks)...", flush=True)
            
            # डिफ़ॉल्ट लोकेटर सेट करें
            code_block_locator = page.locator('#code-block-viewer pre')
            json_content = None
            
            for attempt in range(1, 8):  # कुल 7 बार अटेम्प्ट करेगा (बड़ा टाइमआउट विंडो)
                print(f"[STEP] Checking code block locator (Attempt {attempt}/7)...", flush=True)
                
                # फॉलबैक 1: अगर ID नहीं मिली, तो क्लास और जेनेरिक स्ट्रक्चर का मेल खोजें
                if code_block_locator.count() == 0:
                    print("[INFO] Code Block Fallback 1: Searching via syntax-highlighter pre...", flush=True)
                    code_block_locator = page.locator('pre.fluid-markdown-code-block, pre')
                    
                # फॉलबैक 2: सबसे सेफ तरीका—पेज पर मौजूद कोई भी साधारण 'pre' टैग
                if code_block_locator.count() == 0:
                    print("[INFO] Code Block Fallback 2: Searching via general 'pre' tag...", flush=True)
                    code_block_locator = page.locator('pre')

                # अगर लोकेटर मिल जाता है, तो लाइव डेटा स्ट्रीम को रीड करना शुरू करें
                if code_block_locator.count() > 0:
                    print("[OK] Code block visible, parsing live text size variations...", flush=True)
                    
                    last_length = 0
                    max_check_cycles = 20  # कुल 20 साइकिल तक वेट करेगा ताकि लंबा रिस्पॉन्स कटे नहीं
                    ...
                    for cycle in range(max_check_cycles):
                        ...
                        # अगर टेक्स्ट का साइज बढ़ना बंद हो गया है और उसमें कुछ डेटा आ चुका है
                        if current_length > 0 and current_length == last_length:
                            # 'endswith' की जगह सीधे चेक कर रहे हैं कि ब्लॉक क्लोजिंग ब्रैकेट आ गया है या नहीं
                            if "}" in current_text:
